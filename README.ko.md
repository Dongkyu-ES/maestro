...
**AI 코딩 에이전트를 위한, 프로바이더 중립적인 증거·통제 계층(evidence & control layer).** Warden은 에이전트 루프 자체(Codex CLI, Claude Code, Antigravity/agy — 모두 헤드리스로 구동)는 *빌려 쓰고*, 다른 어디에도 없는 계층을 *소유*합니다: 해시 체인으로 묶인 이벤트 원장(ledger), 재계산 가능한 검증기(verifier), 내용 주소화된 증거(content-addressed evidence), 승급(promotion), 정책(policy), 그리고 운영자 UI. **완료(completion)는 오직 원장에 적재된 증거 위에서 합격 기준(acceptance)을 다시 실행해서만 선언됩니다 — 모델·CLI·점수·리뷰 산문(prose)이 선언하지 않습니다.**

> 로컬, 파일 기반, 단일 운영자(single-operator). 상태는 `.agent/` 아래에 저장됩니다. 호스팅 서비스 없음, SaaS 없음, 자동 push 없음.

**언제 쓰나** — 경계가 분명한(bounded) 코딩 작업을 CLI 에이전트에게 맡기고, 그게 *정말로* 끝났는지 — 에이전트의 말을 믿는 게 아니라 — *알아야* 할 때. Warden에 작업과 합격 기준(테스트 명령)을 주면, 격리된 워크트리(worktree)에서 네이티브 CLI를 실행하고, 그 작업을 변조 감지 가능한 원장에 기록하며, 깨끗한 체크아웃에서 산출된 diff 위로 합격 기준을 *다시 실행*해서 실제로 통과할 때만 `passed`를 알려줍니다. 그 대가로 얻는 것은 모델이 "끝났어요"라고 말하는 대신, **재계산하고 감사(audit)할 수 있는 완료 판정**입니다.

## 왜(Why)

LLM 코딩 에이전트는 *작업을 해내는 것*에는 능하지만 *그게 끝났는지 정직하게 보고하는 것*에는 서툽니다 — 산문으로 스스로 완료를 인증(self-certify)해 버립니다. Warden의 명제: **증거와 게이트를 소유하고, 루프는 빌린다.** 네이티브 CLI가 격리된 git 워크트리 안에서 작업을 수행하고, Warden은 모든 단계를 변조 감지 원장에 기록한 뒤, 깨끗한 체크아웃에서 산출된 증거 위로 합격 기준을 다시 실행해 완료를 판정합니다. **게이트를 재계산할 수 없다면, 그것은 완료가 아닙니다.**
...
npm install
npm run build
npm link            # `warden`를 PATH에 등록 (상태는 여전히 cwd별 .agent/)
warden --version
```
...
warden task add "경계가 분명한 이슈를 조사하고 수정"
...
warden web --port 4317      # 운영자 UI: 원장에서 진실을 재계산하고, 모순을 표시
```
...
- **승인 게이트 적용:** `warden apply propose <run-id>` → `warden approval approve <id>` → `warden apply approved <id>` (먼저 `git apply --check`; 절대 자동 push 안 함).
- **변경성 셸(Mutating shell)** 은 `shell_mutation` 승인을 생성하고, 승인 전까지 실행하지 않습니다.

## 하네스 런타임 (증거 계층)

```bash
warden harness run "<목표>" --executor codex|claude|agy|anthropic-direct
warden skill run <spec.json> --what "<목표>"   # 리서치 → 실행 → 리뷰
warden skill show <runId>                       # 원장에서 완료를 재계산; 모순 표시
warden runtime verify-ledger <runId>            # 해시 체인 변조 검사
warden verifier run --run <runId>               # 재계산 가능한 합격 판정
warden orchestrate serve | run --file <graph.json>   # DAG 데몬 (요청에 담긴 verifyCmd는 거부)
```
...
## Warden Magic — 프로젝트별 의존성 합성 (LLM 의존성을 위한 Tuist)

프로젝트를 분석해 필요한 LLM-의존성 모듈(MCP 서버, 지시문 파일)을 해결하고, 주입한 뒤 실행합니다 — 프로젝트마다 손으로 배선할 필요가 없습니다.

```bash
warden magic plan "<목표>"        # 프로젝트 태그 탐지 + 모듈 해결 (드라이런; 아무것도 주입 안 함)
warden magic catalog              # 모듈 카탈로그 나열 (선언 + 발견)
warden magic apply [--into <dir>] [--executor ...] [--approve-secrets]   # 주입 + 해시 체인 기록
warden magic show <magicRunId>    # 원장에서 주입 기록을 재계산; 모순 표시
warden magic run "<목표>" [--executor ...] [--prove]                     # 주입 후 실행기 실행
```
...
- `src/composition/` — Warden Magic: 탐지 / 카탈로그 / 해결 / 주입 / 원장-증거 / 카나리
- `src/events/ledger.ts` — 해시 체인 런타임 이벤트 원장
- `src/memory/` — 출처-키 기반 메모리 패브릭
- `src/cli.ts` — `warden` CLI; `src/view.ts` — 운영자 웹 UI
