...
import { AGENT_DIR, frontmatter, nowIso, parseFrontmatter, projectRoot, safeJoin, uniqueId, } from '../util.js';
...
export function taskPath(taskId, cwd = process.cwd()) {
...
export function addTask(title, cwd = process.cwd()) {
    initProject(cwd);
    const root = projectRoot(cwd);
    const ts = nowIso();
    ...
    writeFileSync(safeJoin(root, AGENT_DIR, 'tasks', `${id}.md`), frontmatter(meta, body));
    rebuildIndex(root);
    return meta;
}
export function updateTaskStatus(taskId, status, cwd = process.cwd()) {
...
export function updateTask(taskId, fields, cwd = process.cwd()) {
