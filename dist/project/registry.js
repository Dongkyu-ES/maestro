...
import { AGENT_DIR, ensureDir, nowIso, projectRoot, registryPath, safeJoin, slug, writeIfMissing, yaml, } from '../util.js';
...
export function initProject(cwd = process.cwd()) {
    ...
    for (const [rel, content] of files)
        if (writeIfMissing(safeJoin(root, AGENT_DIR, rel), content))
            created.push(`${AGENT_DIR}/${rel}`);
    for (const dir of ['tasks', 'runs', 'logs', 'cache', 'approvals', 'promotions', 'worktrees'])
        ensureDir(safeJoin(root, AGENT_DIR, dir));
    rebuildIndex(root);
    return created;
}
export function addProject(rootPath = process.cwd()) {
