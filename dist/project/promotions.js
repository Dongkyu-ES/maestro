...
import { AGENT_DIR, ensureDir, nowIso, projectRoot, readYaml, safeJoin, slug, uniqueId, writeIfMissing, } from '../util.js';
export function listPromotions(cwd = process.cwd()) {
...
export function classifyRunPromotions(runDir, runId, cwd = process.cwd()) {
    const root = projectRoot(cwd);
    const read = (rel) => (existsSync(join(runDir, rel)) ? readFileSync(join(runDir, rel), 'utf8') : '');
    const review = read('review.md');
    const result = read('result.md');
    const runMeta = existsSync(join(runDir, 'run.yaml'))
        ? readYaml(join(runDir, 'run.yaml'))
        : {};
...
export function resolvePromotion(id, status, cwd = process.cwd()) {
    const root = projectRoot(cwd);
...
export function applyApprovedPromotion(id, cwd = process.cwd()) {
    const root = projectRoot(cwd);
    const p = safeJoin(root, AGENT_DIR, 'promotions', `${id}.json`);
    if (!existsSync(p))
        throw new Error(`promotion ${id} not found`);
    ...
    if (rec.status !== 'approved')
        throw new Error(`promotion ${id} is not approved (status: ${rec.status})`);
