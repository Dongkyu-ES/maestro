...
export function codexSessionRegistryPath(agentDir) {
    return join(agentDir, 'runtime', 'codex-sessions.json');
}
export function readCodexSessionRegistry(agentDir) {
    const path = codexSessionRegistryPath(agentDir);
    if (!existsSync(path))
        return { schema_version: 1, sessions: [] };
    return JSON.parse(readFileSync(path, 'utf8'));
}
export function writeCodexSessionRegistry(agentDir, registry) {
    const path = codexSessionRegistryPath(agentDir);
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify(registry, null, 2));
}
export function upsertCodexSession(agentDir, record) {
