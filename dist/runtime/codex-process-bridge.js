...
import { sha256Text, upsertCodexSession } from './codex-session-registry.js';
export function discoverCodexTranscriptPath(threadId = process.env.CODEX_THREAD_ID, codexHome = process.env.CODEX_HOME || join(homedir(), '.codex')) {
...
export function shouldAutoAttachCurrentCodexTranscript(cwd) {
    return Boolean(process.env.CODEX_THREAD_ID && cwd === process.cwd());
}
export function createCodexLaunchProof(options) {
    mkdirSync(options.runDir, { recursive: true });
    const detected = detectCodexCli(options.cwd);
    const notes = [];
    let status = detected.available ? 'unproven' : 'unsupported';
    const transcriptPath = options.liveTranscriptPath ||
        (options.autoAttachCurrentSession && shouldAutoAttachCurrentCodexTranscript(options.cwd)
            ? discoverCodexTranscriptPath()
            : undefined);
    let transcriptSha;
    ...
    if (detected.available) {
        try {
            doctor = execFileSync('codex doctor', { cwd: options.cwd, shell: true, encoding: 'utf8', timeout: 10000 });
        }
        catch (err) {
            doctor = String(err.stdout || err.stderr || err.message || err);
            notes.push('codex doctor returned non-zero; preserved as evidence.');
        }
    }
    const session = upsertCodexSession(options.agentDir, {
