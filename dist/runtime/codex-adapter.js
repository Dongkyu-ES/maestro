...
function discoverCurrentTranscript(cwd) {
    const threadId = process.env.CODEX_THREAD_ID;
    if (!threadId || cwd !== process.cwd() || !/^[A-Za-z0-9-]+$/.test(threadId))
        return {};
...
export class CodexCliAdapter {
    ...
    async *launch(request) {
        ...
        if (request.metadata?.evidenceDir && typeof request.metadata.evidenceDir === 'string') {
            ...
            writeFileSync(join(request.metadata.evidenceDir, rel), JSON.stringify({
                schema_version: 1,
                verb: 'launch',
                adapter_kind: 'codex',
                detected,
...
function lifecycleEvent(sessionId, verb) {
    return {
        runId: sessionId,
        sessionId,
        source: 'codex-adapter',
