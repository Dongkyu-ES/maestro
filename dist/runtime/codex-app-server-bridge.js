...
export class CodexAppServerJsonRpcBridge {
    ...
    async proveFork(target) {
        ...
        return writeProof(target, 'fork', {
            method: 'thread/fork',
            params: { threadId: target.threadId, ephemeral: target.forkEphemeral || undefined },
            response,
        }, ok, ok ? `forked ${target.threadId} -> ${thread?.id}` : 'thread/fork response did not prove forkedFromId linkage');
    }
    async proveInterrupt(target) {
        if (!target.turnId)
            return writeProof(target, 'interrupt', { method: 'turn/interrupt', params: { threadId: target.threadId, turnId: null }, response: null }, false, 'interrupt requires target turnId');
        ...
        return writeProof(target, 'interrupt', {
...
export class CodexAppServerStdioTransport {
    ...
    pending = new Map();
    buffer = '';
    constructor(command = 'codex', args = ['app-server', '--listen', 'stdio://'], requestTimeoutMs = 30000) {
        this.requestTimeoutMs = requestTimeoutMs;
        this.child = spawn(command, args, { stdio: ['pipe', 'pipe', 'pipe'] });
        this.child.stdout.on('data', (chunk) => this.onData(chunk.toString()));
        this.child.stderr.on('data', (chunk) => {
            if (process.env.DEBUG_CODEX_APP_SERVER_BRIDGE)
...
function hasTurnStatus(response, turnId, status) {
    const turns = response?.thread?.turns;
    return Array.isArray(turns) && turns.some((turn) => turn?.id === turnId && turn?.status === status);
}
function writeProof(target, verb, raw, ok, reason) {
