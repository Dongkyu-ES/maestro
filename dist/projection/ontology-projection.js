...
function agentKindForSource(source, payloadAdapterKind) {
    if (typeof payloadAdapterKind === 'string' && payloadAdapterKind)
        return payloadAdapterKind;
...
class GraphBuilder {
    nodes = new Map();
    edges = new Map();
    upsertNode(id, space, type, label, sourceEventId, attrs = {}) {
        const existing = this.nodes.get(id);
        if (existing) {
            if (!existing.source_event_ids.includes(sourceEventId))
                existing.source_event_ids.push(sourceEventId);
    ...
    upsertEdge(from, relation, to, sourceEventId) {
    ...
    finish(asOf) {
        return {
            ...
            edges: [...this.edges.values()].sort((a, b) => a.from.localeCompare(b.from) || a.relation.localeCompare(b.relation) || a.to.localeCompare(b.to)),
        };
    }
}
/**
 ...
 * tampered ledger.
 */
export function rebuildOntologyProjection(events) {
    ...
    for (const event of events) {
        ...
        if (agentKind) {
            ...
            const executors = executorsByRun.get(event.run_id) ?? new Set();
...
export function findOntologyNode(projection, id) {
...
export function ontologyNeighbors(projection, fromId) {
 ...
 */
export function composeOntologyProjections(base, ...overlays) {
    const nodes = new Map();
    const edges = new Map();
    for (const fragment of [base, ...overlays]) {
        ...
        for (const edge of fragment.edges) {
            ...
            if (existing)
                ...
                edges.set(key, { ...edge, source_event_ids: [...edge.source_event_ids] });
        }
    }
    return {
        schema_version: 1,
        ...
        edges: [...edges.values()].sort((a, b) => a.from.localeCompare(b.from) || a.relation.localeCompare(b.relation) || a.to.localeCompare(b.to)),
    };
}
