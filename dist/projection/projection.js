...
export function rebuildRuntimeProjection(events) {
    ...
    for (const event of events) {
        ...
        const label = typeof event.payload.runtime_label === 'string'
            ? event.payload.runtime_label
            : typeof event.payload.adapter_kind === 'string'
                ? event.payload.adapter_kind
                : undefined;
        if (label)
            run.labels = [...new Set([...run.labels, label])].sort();
...
export function findProjectedRun(projection, runId) {
