...
export function policyRelationForDecision(decision) {
    if (decision === 'allow')
        return 'permits';
    if (decision === 'deny')
        return 'denies';
...
export function rebacForRisk(risk) {
    switch (risk) {
        case 'read_only':
 ...
 */
export function projectToolPolicyDecisions(inputs) {
    const nodes = new Map();
    const edges = new Map();
    ...
    for (const group of groups.values()) {
        ...
        const capabilityProvenance = [
            ...new Set(group.filter((i) => i.decision === input.decision && i.risk === input.risk).flatMap(provenanceOf)),
        ];
        const toolId = `resource:tool:${input.tool}`;
        const policyId = `policy:tool-rule:${input.tool}`;
