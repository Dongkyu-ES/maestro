...
import { validateMemoryWrite } from './records.js';
...
export function memoryFabricPath(agentDir) {
    return join(agentDir, 'memory', 'fabric.json');
}
export function readMemoryFabric(agentDir) {
    ...
    try {
        if (statSync(path).size > MAX_FABRIC_BYTES)
            return { schema_version: 1, facts: [] };
...
export function writeMemoryFabric(agentDir, store) {
    const path = memoryFabricPath(agentDir);
...
export function appendMemoryFact(agentDir, fact) {
 ...
 */
export function factFromWriteRecord(record) {
    return {
        id: record.memory_id,
...
export function appendFactFromWriteRecord(agentDir, record) {
 ...
 */
export function markFactsVerifiedByEvents(agentDir, verifiedEventIds, at) {
    const verified = new Set(verifiedEventIds);
    const store = readMemoryFabric(agentDir);
    const stamped = [];
    for (const fact of store.facts) {
        if (fact.source_event_ids.length === 0)
            continue;
...
export function recommendModules(agentDir, goalKey) {
