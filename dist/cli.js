...
import { addProject, addTask, applyApprovedPromotion, applyApprovedProposal, cancelRun, cleanupWorktrees, collectRun, createApproval, createRun, initProject, latestRunId, listApprovals, listProjects, listPromotions, listTasks, loadIndex, proposeApply, rebuildIndex, rebuildRuntimeProjectionStore, reconcileRuns, removeProject, resolveApproval, resolvePromotion, runPath, startRun, taskPath, updateTask, } from './core.js';
...
import { loadSkillSpecFromJson, projectSkillRun, recomputeCompletionFromLedger, runOrchestratorSkill, writeSkillLaunchMarker, } from './harness/orchestrator-skill.js';
...
import { reviewCustodyKey, reviewCustodySignature, reviewProvenanceKey, reviewProvenanceSignature, sha256Text, } from './util.js';
 ...
 */
export function listSkillSpecs() {
...
function safeReviewRelPath(root, artifactPath) {
    ...
    if (!existsSync(full))
        throw new Error(`review artifact is missing: ${artifactPath}`);
    const rel = relative(realpathSync(root), realpathSync(full)).replaceAll('\\', '/');
    if (rel === '..' || rel.startsWith('../'))
        throw new Error('review artifact_path escapes the repo');
    return artifactPath;
}
function readReviewArtifact(root, artifactPath) {
    const relPath = safeReviewRelPath(root, artifactPath);
    return readFileSync(join(root, relPath), 'utf8');
}
function readReviewJsonArtifact(root, artifactPath) {
    const relPath = safeReviewRelPath(root, artifactPath);
...
async function readBody(req) {
    ...
    return parseQuery(Buffer.concat(chunks).toString());
}
    ...
    try {
        ...
        if (cmd === 'project' && sub === 'list') {
            for (const p of listProjects())
                console.log(`${p.id}\t${p.root_path}`);
            return;
        }
        if (cmd === 'project' && sub === 'show') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro project show <id>');
        ...
        if (cmd === 'project' && sub === 'remove') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro project remove <id>');
            removeProject(id);
            return;
        }
        ...
        if (cmd === 'task' && sub === 'add') {
            ...
            if (!title)
                throw new Error('usage: maestro task add "title"');
        ...
        if (cmd === 'task' && sub === 'show') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro task show <task-id>');
            const p = taskPath(id);
            if (!existsSync(p))
        ...
        if (cmd === 'task' && sub === 'status') {
            ...
            const status = rest[1];
            if (!id || !status)
                throw new Error('usage: maestro task status <task-id> <status>');
        ...
        if (cmd === 'task' && sub === 'update') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro task update <task-id> [--title title] [--status status]');
            console.log(JSON.stringify(updateTask(id, { title: arg('--title'), status: arg('--status') }), null, 2));
            return;
        }
        if (cmd === 'task' && sub === 'archive') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro task archive <task-id>');
        ...
        if (cmd === 'run' && sub === 'create') {
            ...
            if (!id)
                throw new Error('usage: maestro run create <task-id> [--mode roles|multi] [--executor command|codex|omx|agy] [--max-workers N] [--command cmd]');
            const mode = (arg('--mode', has('--multi') ? 'multi' : has('--roles') ? 'roles' : 'basic') || 'basic');
            const maxWorkers = Number(arg('--max-workers', '2'));
            const command = arg('--command');
            const executor = (arg('--executor', 'command') || 'command');
        ...
        if (cmd === 'run' && sub === 'start') {
            ...
            if (!id)
                throw new Error('usage: maestro run start <run-id> [--command cmd] [--sandbox mode] [--timeout-ms N]');
        ...
        if (cmd === 'run' && sub === 'collect') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro run collect <run-id>');
        ...
        if (cmd === 'run' && sub === 'cancel') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro run cancel <run-id>');
            cancelRun(id);
        ...
        if (cmd === 'run' && sub === 'native-evidence-smoke') {
            const task = arg('--task');
            if (!task)
                throw new Error('usage: maestro run native-evidence-smoke --task <fixture-task> [--timeout-ms N]');
            const timeoutArg = arg('--timeout-ms');
            ...
            if (report.verification.decision !== 'PASS')
                process.exitCode = 2;
            return;
        }
        if (cmd === 'harness' && sub === 'run') {
            ...
            const goal = rest
                ...
                .trim();
            if (!goal)
                throw new Error('usage: maestro harness run <goal> [--executor codex|claude|agy|anthropic-direct|<custom>] [--executor-bin <path>] [--acceptance-file <path>]');
            ...
            if (executorKind === 'anthropic-direct') {
                ...
                const detect = (bin) => bin.includes('/') ? existsSync(bin) : detectExternalCli(bin).available;
        ...
        if (cmd === 'loop' && sub === 'run') {
            const acceptanceFile = arg('--acceptance-file');
            const goal = rest
                .filter((item, index) => !['--acceptance-file', '--max-iters', '--stall', '--executor-bin', '--verify-cmd'].includes(item) &&
                !['--acceptance-file', '--max-iters', '--stall', '--executor-bin', '--verify-cmd'].includes(rest[index - 1]))
                .join(' ')
                .trim();
        ...
        if (cmd === 'context' && sub === 'verify') {
            ...
            if (!id)
                throw new Error('usage: maestro context verify --run <run-id>');
        ...
        if (cmd === 'runtime' && sub === 'verify-ledger') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro runtime verify-ledger <run-id>');
        ...
        if (cmd === 'runtime' && sub === 'full-target-gate') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro runtime full-target-gate <run-id> [--append-pass-event]');
        ...
        if (cmd === 'runtime' && sub === 'm8-boundary-evidence') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro runtime m8-boundary-evidence <run-id>');
            console.log(JSON.stringify(appendM8BoundaryEvidence({ root: process.cwd(), agentDir: '.agent', runId: id }), null, 2));
            return;
        }
        ...
        if (cmd === 'runtime' && sub === 'ui-agreement') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro runtime ui-agreement <run-id>');
        ...
        if (cmd === 'runtime' && sub === 'verify-full-target') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro runtime verify-full-target <run-id> [--append-verified-event]');
        ...
        if (cmd === 'verifier' && sub === 'run') {
            const id = arg('--run') || rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro verifier run --run <run-id>');
        ...
        if (cmd === 'runtime' && sub === 'sign-review') {
            ...
            if (!existsSync(gatePath))
                throw new Error(`missing review gate: ${gatePath}`);
            const key = reviewProvenanceKey();
            if (!key)
                throw new Error('no signing key configured: set AGENT_REVIEW_HMAC_KEY or create ~/.dominic_orchestration/review-signing.key');
            ...
            if (String(custody || '') !== 'reviewer-ci')
                throw new Error('review custody is required: pass --custody reviewer-ci or set AGENT_REVIEW_CUSTODY=reviewer-ci; local/non-CI signatures are diagnostic only');
            const custodyKey = reviewCustodyKey();
            if (!custodyKey)
                throw new Error('no review custody key configured: set AGENT_REVIEW_CUSTODY_HMAC_KEY or create ~/.dominic_orchestration/review-custody.key');
            const reviewSessionId = arg('--review-session') ||
            ...
            if (!custodyIssuer || !reviewSessionId || !reviewerAgentId)
                throw new Error('review custody metadata is required: provide --custody-issuer and --review-session (or AGENT_REVIEW_CUSTODY_ISSUER/AGENT_REVIEW_SESSION_ID) plus reviewer agent id');
        ...
        if (cmd === 'approvals' && !sub) {
            for (const a of listApprovals())
                console.log(`${a.id}\t${a.status}\t${a.risk}\t${a.type}\t${a.summary}`);
            return;
        }
        if (cmd === 'approval' && (sub === 'approve' || sub === 'reject')) {
            const id = rest[0];
            if (!id)
                throw new Error(`usage: maestro approval ${sub} <id>`);
        ...
        if (cmd === 'approval' && sub === 'request') {
            ...
            if (!runId)
                throw new Error('usage: maestro approval request <run-id>');
            console.log(JSON.stringify(createApproval(runId, arg('--type', 'manual'), 'medium', arg('--summary', 'manual approval request')), null, 2));
            return;
        }
        if (cmd === 'apply' && sub === 'propose') {
            const id = rest[0] || latestRunId();
            if (!id)
                throw new Error('usage: maestro apply propose <run-id>');
        ...
        if (cmd === 'apply' && sub === 'approved') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro apply approved <approval-id>');
        ...
        if (cmd === 'report' && sub === 'github-projects') {
            ...
            if (!existsSync(script))
                throw new Error(`report script missing: ${script}`);
            const scriptArgs = [script];
            const githubDir = arg('--github-dir');
            const outDir = arg('--out-dir');
            const maxProjects = arg('--max-projects');
            if (githubDir)
                scriptArgs.push('--github-dir', githubDir);
            if (outDir)
                scriptArgs.push('--out-dir', outDir);
            if (maxProjects)
                scriptArgs.push('--max-projects', maxProjects);
        ...
        if (cmd === 'promotions' && !sub) {
            for (const p of listPromotions())
                console.log(`${p.id}\t${p.status}\t${p.target_type}\t${p.reason}`);
            return;
        }
        if (cmd === 'promotion' && (sub === 'approve' || sub === 'reject')) {
            const id = rest[0];
            if (!id)
                throw new Error(`usage: maestro promotion ${sub} <id>`);
        ...
        if (cmd === 'promotion' && sub === 'apply') {
            const id = rest[0];
            if (!id)
                throw new Error('usage: maestro promotion apply <id>');
        ...
        if (cmd === 'promotion' && sub === 'verify-causal') {
            ...
            if (!goal || !promotionFile)
                throw new Error('usage: maestro promotion verify-causal --goal <g> --promotion-file <path> [--executor codex|claude|agy] [--executor-bin <path>] [--promotion-id <id>]');
        ...
        if (cmd === 'provider' && sub === 'conformance') {
            if (!has('--all'))
                throw new Error('usage: maestro provider conformance --all');
        ...
        if (cmd === 'skill' && sub === 'run') {
            ...
            const what = arg('--what');
            if (!specPath || !what)
                throw new Error('usage: maestro skill run <spec.json> --what "<goal>"');
            ...
            const json = JSON.parse(readFileSync(specPath, 'utf8'));
            ...
            console.log(`AUTHORITATIVE (ledger recompute, report field is display-only): completion=${recomputed.completion} (${recomputed.reason})`);
            if (recomputed.completion === 'failed')
                process.exitCode = 2;
            return;
        }
        if (cmd === 'skill' && sub === 'show') {
            ...
            if (!runId)
                throw new Error('usage: maestro skill show <runId>');
            const projection = projectSkillRun({ root: process.cwd(), runId });
            console.log(JSON.stringify(projection, null, 2));
            if (projection.contradiction) {
                console.log('CONTRADICTION: stored/display completion disagrees with the authoritative ledger recompute — trust the recompute, not the report.');
                process.exitCode = 2;
            }
            return;
        }
        if (cmd === 'magic' && sub === 'plan') {
            ...
            if (!goal)
                throw new Error('usage: maestro magic plan "<goal>"');
        ...
        if (cmd === 'magic' && sub === 'apply') {
            ...
            if (!verification.integrityOk || (ledgerCheck.found && !ledgerCheck.reproduced))
                process.exitCode = 2;
            return;
        }
        if (cmd === 'magic' && sub === 'run') {
            const goal = firstNonFlag(rest);
            if (!goal)
                throw new Error('usage: maestro magic run "<goal>" [--executor claude|codex|agy] [--approve-secrets]');
            const executorLabel = arg('--executor') ?? 'claude';
            const magicRegistry = defaultExecutorRegistry();
            if (!magicRegistry.has(executorLabel))
                throw new Error(`unknown executor: ${executorLabel} (use codex|claude|agy)`);
        ...
        if (cmd === 'magic' && sub === 'show') {
            ...
            if (!magicRunId)
                throw new Error('usage: maestro magic show <magicRunId> [--into <dir>] [--executor ...]');
            const into = arg('--into') ?? process.cwd();
            const executor = arg('--executor') ?? 'claude';
            const runDir = join(into, '.agent', 'magic-runs', magicRunId);
        ...
        if (cmd === 'quality' && sub === 'gate') {
            ...
            console.log('ADVISORY ONLY: this gate cannot mark a run or milestone complete. Completion authority = M7 ledger/diff verifier.');
            console.log(JSON.stringify(report, null, 2));
            if (report.decision !== 'PASS')
                process.exitCode = 2;
            return;
        ...
        if (cmd === 'orchestrate' && sub === 'run') {
            const file = arg('--file');
            if (!file)
                throw new Error('usage: maestro orchestrate run --file <graph.json> [--reconcile] [--verify-cmd CMD]');
...
async function serveWeb() {
    ...
    const unsafeHost = has('--unsafe-host');
    if (!['127.0.0.1', 'localhost'].includes(host) && !unsafeHost)
        throw new Error('maestro web only binds loopback hosts by default; pass --unsafe-host with --auth-token to acknowledge remote command/control risk');
    const authToken = arg('--auth-token') || process.env.AGENT_WEB_TOKEN || '';
    if (unsafeHost && !authToken)
        throw new Error('--unsafe-host requires --auth-token or AGENT_WEB_TOKEN because web controls can execute local commands');
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            const providedAuth = () => url.searchParams.get('auth') || String(req.headers['x-agent-auth'] || '') || cookieValue(req, 'agent_auth');
            const requireRemoteAuth = (provided) => {
                if (!authToken)
                    return;
                const ok = provided.length === authToken.length && timingSafeEqual(Buffer.from(provided), Buffer.from(authToken));
                if (!ok)
                    throw new Error('invalid auth token');
            };
            const requirePageAuth = () => {
                if (!authToken)
                    return;
            ...
            if (req.method === 'POST' && taskUpdateMatch) {
                const body = await requirePostAuth();
                updateTask(taskUpdateMatch[1], { title: body.title || undefined, status: body.status || undefined });
                redirect(res);
                return;
            }
            ...
            if (req.method === 'POST' && url.pathname === '/api/runs') {
                ...
                createRun(body.taskId, { mode: (body.mode || 'basic'), executor: executor, source: 'web' });
                redirect(res);
                return;
            }
            if (req.method === 'POST' && url.pathname === '/api/skill-runs') {
                ...
                if (!what)
                    throw new Error('a skill run needs a one-line goal (what)');
                ...
                if (what.startsWith('--'))
                    throw new Error('goal (what) must not start with "--"');
                const chosen = listSkillSpecs().find((s) => s.id === body.specId);
                if (!chosen)
                    throw new Error('unknown skill spec');
                // Fail fast: confirm the spec is a real, loadable skill before spawning a detached child.
                const reg = defaultExecutorRegistry();
                loadSkillSpecFromJson(JSON.parse(readFileSync(chosen.path, 'utf8')), {
                    codex: reg.resolve('codex'),
                    claude: reg.resolve('claude'),
                    agy: reg.resolve('agy'),
                });
                ...
                const child = spawn(process.execPath, [fileURLToPath(import.meta.url), 'skill', 'run', chosen.path, '--what', what, '--run-id', runId], { cwd: root, detached: true, stdio: ['ignore', logFd, logFd] });
            ...
            if (url.pathname.startsWith('/run/'))
                res.end(renderRun(decodeURIComponent(url.pathname.slice(5))));
            else if (url.pathname.startsWith('/skill/'))
                res.end(renderSkillRun(decodeURIComponent(url.pathname.slice(7))));
            else if (url.pathname === '/review-gate')
                res.end(renderReviewGate(process.cwd()));
            else if (url.pathname === '/')
                res.end(renderHtml(process.cwd(), csrfToken, '', listSkillSpecs()));
            else {
                res.statusCode = 404;
                res.end('not found');
            }
