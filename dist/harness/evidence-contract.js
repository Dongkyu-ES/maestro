...
export function validateEvidenceContract(definition = EVIDENCE_CONTRACT) {
    const errors = [];
    const allowedStatuses = new Set(CONTRACT_STATUSES);
    ...
    for (const status of CONTRACT_STATUSES) {
        if (!definition.statusVocabulary.includes(status))
            errors.push(`missing status: ${status}`);
    }
        ...
        for (const field of type.fields) {
            for (const ref of field.references ?? []) {
                if (!typeNames.has(ref))
                    errors.push(`${type.name}.${field.name} references missing type ${ref}`);
            }
        }
    }
    for (const typeName of ['ExecutorContract', 'ToolIntentContract']) {
...
export function assertEvidenceContractValid(definition = EVIDENCE_CONTRACT) {
