export function runRuntimeParityGate(capabilities) {
    const runtimeKinds = new Set(capabilities.map((cap) => cap.kind));
    const requiredKinds = ['codex', 'omx', 'agy', 'shell'];
    const allKindsPresent = requiredKinds.every((kind) => runtimeKinds.has(kind));
