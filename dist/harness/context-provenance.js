...
function fileHash(root, ref) {
    return sha256(readFileSync(join(root, ref)));
}
function runDir(root, agentDir, runId) {
    return join(root, agentDir, 'runs', runId);
}
function existingContextRefs(root, agentDir, runId) {
...
export function writeContextProvenanceBundle(options) {
...
export function verifyContextProvenance(options) {
    ...
    const projection = ledgerValid ? rebuildRuntimeProjection(events) : { schema_version: 1, rebuilt_at: new Date().toISOString(), runs: [] };
    const projected = findProjectedRun(projection, options.runId);
    const checks = {
        ledger_valid: ledgerValid,
        bundle_bound_to_current_head: bundle.ledger_head.run_id === currentHead.run_id &&
