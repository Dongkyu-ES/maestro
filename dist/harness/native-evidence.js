...
export function verifyNativeEvidenceRun(options) {
    const agentDir = options.agentDir || '.agent';
    const runDir = join(options.root, agentDir, 'runs', options.runId);
    const artifact = safeReadJson(join(runDir, 'native-evidence.json'));
    ...
    const artifactHashesMatch = Boolean(artifact?.raw_artifacts?.length &&
    ...
    const processLog = safeReadJson(join(runDir, 'executor.process.json'));
    ...
    const checks = {
        ...
        diff_matches_effect: Boolean(artifact?.diff_sha256 === sha256Buffer(diff) &&
        ...
        ledger_refs_native_evidence: events.some((event) => event.source === 'codex-adapter' && event.artifact_refs.includes('native-evidence.json') && event.artifact_refs.includes('native-diff.patch')),
    };
    const report = {
        schema_version: 1,
