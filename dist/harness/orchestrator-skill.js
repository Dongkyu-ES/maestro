...
import { appendRuntimeEvent, readRuntimeEvents, runtimeLedgerHeadHash, validateRuntimeLedger, } from '../events/ledger.js';
export { materializeEvidenceInto, resolveEvidenceArtifact, storePhaseArtifact, } from './evidence-store.js';
import { materializeEvidenceInto, storePhaseArtifact } from './evidence-store.js';
import { removeWorktreeAndBranch, runIsolatedWorker, runWorkersConcurrently, } from './orchestrator.js';
import { applyCompositionToWorktree, verifyInjection, } from '../composition/inject.js';
...
function interpolateGoal(template, input) {
    return template.replaceAll('{what}', input.what);
}
function phaseFromJson(phaseId, phase, executors) {
    if (!phase)
        throw new Error(`missing phase: ${phaseId}`);
    if (!(phase.executor in executors))
        throw new Error(`unknown executor: ${phase.executor}`);
    return {
...
export function loadSkillSpecFromJson(json, executors) {
    if (!json.phases)
...
export function compileSkillToGraphTemplate(spec, input) {
    return [
        {
            id: 'research',
...
const PHASE_IDS = ['research', 'execute', 'review'];
function isSupported(result) {
    return result.state === 'completed' && result.verifierStatus === 'supported';
}
function toPhaseResult(result, phase) {
    return {
        phase,
...
function workerIdFor(runId, phase) {
    return `${phase}-${sha256Hex(runId).slice(0, 12)}`;
}
function readWorkerLedgerHead(result) {
    if (!result.runDir)
        return undefined;
...
function completionFromReview(review) {
    if (review?.nodeState === 'supported')
        return 'passed';
    if (review?.nodeState === 'skipped')
        return 'skipped';
    return 'failed';
}
...
export function skillRunDir(root, runId) {
...
export function recomputeCompletion(spec, report, opts) {
    if (!spec.acceptance) {
        return {
 ...
 */
export function recomputeCompletionFromLedger(spec, opts) {
    ...
    if (evidenceEvent && evidenceEvent.payload.mode === 'diff') {
        const recorded = evidenceEvent.payload.files ?? [];
...
function executeCandidateWorkerId(runId, candidate) {
...
async function runExecuteFanOut(opts) {
    ...
    for (const spec of workerSpecs)
        opts.createdWorkerIds.push(spec.workerId);
    ...
    if (!winnerStored)
        throw new Error('winner selected without stored evidence');
...
async function runExecuteRefinement(opts) {
    ...
    if (!chosenStored)
        throw new Error('refinement winner selected without stored evidence');
...
export async function runOrchestratorSkill(spec, input) {
    ...
    if ((spec.phases.execute.evidence ?? 'artifact') === 'diff' &&
    ...
    for (const phase of PHASE_IDS) {
        ...
        const result = await runIsolatedWorker({
            ...
            beforeExecute: phase === 'execute' && spec.inject
                ? ((injectSpec, pinnedTest) => (worktreePath) => {
        ...
        if (phaseResult.nodeState !== 'supported')
            continue;
        ...
        if (!spec.phases[phase].acceptArtifact)
            continue;
        try {
            const ref = storePhaseArtifact({
                root: input.root,
    ...
    if (!ledgerHead)
        throw new Error('runOrchestratorSkill could not bind a worker ledger head');
    const review = phases.find((phase) => phase.phase === 'review');
    const execute = phases.find((phase) => phase.phase === 'execute');
    const reconstruction = executeEvidenceMode === 'diff' && executeBaseCommit ? { root: input.root, baseCommit: executeBaseCommit } : undefined;
    const acceptance = spec.acceptance && execute?.nodeState === 'supported'
...
function toSerializableSpec(spec) {
    const phase = (id) => ({
...
export function skillRunStatus(root, runId) {
    const dir = skillRunDir(root, runId);
    if (existsSync(join(dir, 'skill-run-report.json')))
        return 'final';
 ...
 */
export function listSkillRunSummaries(root) {
 ...
 */
export function projectSkillRun(opts) {
    const dir = skillRunDir(opts.root, opts.runId);
    const report = JSON.parse(readFileSync(join(dir, 'skill-run-report.json'), 'utf8'));
    const specJson = JSON.parse(readFileSync(join(dir, 'skill-spec.json'), 'utf8'));
    const spec = {
        id: specJson.id,
    ...
    let authoritativeCompletion = 'failed';
    let reason;
    try {
