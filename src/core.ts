...
export function collectRun(runId: string, cwd = process.cwd()): RunMeta {
  ...
  createPromotionProposal(runDir, runMeta.id);
  if (decision !== 'pass')
    createApproval(
      runMeta.id,
...
function createPromotionProposal(runDir: string, runId: string): void {
  const text = existsSync(join(runDir, 'review.md')) ? readFileSync(join(runDir, 'review.md'), 'utf8') : '';
  if (!/System Patch Suggestions\n(?!None\.)/s.test(text)) return;
  const ts = nowIso();
  const proposalPath = join(runDir, 'promotions.md');
  const rec: PromotionRecord = {
    schema_version: 1,
    id: uniqueId('promotion', runId),
    run_id: runId,
    target_type: 'memory',
    status: 'proposed',
    reason: 'Review suggested durable system patch.',
    target_path: '.agent/memory/project-facts.md',
    proposal_path: proposalPath,
    created_at: ts,
    updated_at: ts,
  };
  writeFileSync(proposalPath, `# Promotion Proposal\n\nReason: ${rec.reason}\n\nReview excerpt is in review.md.\n`);
