...
import { type RuntimeProjection, rebuildRuntimeProjection } from '../projection/projection.js';
...
function listTasksForIndex(root: string): TaskMeta[] {
  ...
  return readdirSync(dir)
    ...
    .map((f) => {
      ...
      return {
        ...
        status: (meta.status || 'ready') as TaskMeta['status'],
...
function listApprovalsForIndex(root: string): ApprovalRecord[] {
...
function listPromotionsForIndex(root: string): PromotionRecord[] {
  const dir = safeJoin(root, AGENT_DIR, 'promotions');
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((f) => f.endsWith('.json'))
    .map((f) => JSON.parse(readFileSync(join(dir, f), 'utf8')) as PromotionRecord);
}
...
): { events: RuntimeEventEnvelope[]; migrated: number } {
  let migrated = 0;
