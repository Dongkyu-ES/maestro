...
import { createApprovalInternal, listApprovals } from './approvals.js';
import {
  AGENT_DIR,
  type ApprovalRecord,
  type RunMeta,
...
function runPath(runId: string, cwd = process.cwd()): string {
...
export function proposeApply(runId: string, cwd = process.cwd()): ApprovalRecord {
  ...
  const run = readYaml(join(rDir, 'run.yaml')) as unknown as RunMeta;
