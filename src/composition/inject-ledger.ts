...
import { type InjectedFile, type InjectionAdapter, type InjectionManifest, recomputeInjectionFiles } from './inject.js';
 ...
 * purely from the ledgered inputs and compares it to the recorded event. Two complementary guards:
...
export function recomputeInjectionFromLedger(
  ...
  if (!last) {
    return { ledgerValid: true, found: false, reproduced: false, recorded: [], expected: [], reason: 'no composition.injected event in ledger' };
  }
