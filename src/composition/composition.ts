...
export function buildCompositionPlan(options: {
  ...
  runId: string;
  taskId: string;
  preferredRuntime: 'command' | 'codex' | 'omx' | 'agy';
  mode: string;
}): CompositionPlan {
  ...
  const runtime =
    options.preferredRuntime === 'command' ? 'primitive_shell' : `${options.preferredRuntime}_adapter_unproven`;
  const plan: CompositionPlan = {
    ...
    modules: {
      ...
      agents_md_stack: agentsStack.map((file) =>
        file.startsWith(options.root) ? file.slice(options.root.length + 1) : file,
      ),
