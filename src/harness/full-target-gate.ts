...
export interface FullTargetRequirement {
  name: string;
  status: 'PASS' | 'FAIL';
  evidence: string;
}
...
function eventWith(
  ...
  type: string,
  predicate: (event: RuntimeEventEnvelope) => boolean = () => true,
...
function approvedApprovalArtifact(runDir: string, event: RuntimeEventEnvelope): boolean {
  ...
  return event.artifact_refs.some((ref) => {
    ...
    try {
      const artifact = JSON.parse(readFileSync(join(runDir, ref), 'utf8')) as {
        id?: unknown;
        run_id?: unknown;
        status?: unknown;
      };
...
function approvedBoundary(
  ...
  runDir: string,
  action: string,
): { approvalId: string; requestSequence: number; decisionSequence: number } | undefined {
...
function approvedRuntimeAction(
  events: RuntimeEventEnvelope[],
  runDir: string,
  boundary: { approvalId: string; decisionSequence: number } | undefined,
  action: string,
): boolean {
...
function uiSmokePasses(runDir: string, runId: string): boolean {
  ...
  try {
    const artifact = JSON.parse(readFileSync(join(runDir, 'ui-render-smoke.json'), 'utf8')) as {
      status?: unknown;
      run_id?: unknown;
      checks?: Record<string, unknown>;
    };
    return (
      artifact.status === 'PASS' && artifact.run_id === runId && Object.values(artifact.checks || {}).every(Boolean)
    );
  } catch {
    return false;
  }
...
export function writeFullTargetGateArtifact(options: {
  ...
  agentDir: string;
  runId: string;
  appendPassEvent?: boolean;
}): FullTargetGateArtifact {
  ...
  const projection = ledgerValid
    ...
    : { schema_version: 1 as const, rebuilt_at: new Date().toISOString(), runs: [] };
  const projected = findProjectedRun(projection, options.runId);
  const codexSession = eventWith(
    events,
  ...
  const requirements: FullTargetRequirement[] = [
    req(
      'web goal input',
      Boolean(eventWith(events, 'goal.received', (event) => event.source === 'web')),
      'goal.received must originate from the web control surface',
    ),
    req(
      'sandbox context',
      hasArtifact(runDir, 'context.md') && hasArtifact(runDir, 'prompt.md'),
      'context.md and prompt.md exist in the run sandbox',
    ),
    req(
      'composition',
      Boolean(eventWith(events, 'composition.resolved', (event) => event.artifact_refs.includes('composition.json'))) &&
        hasArtifact(runDir, 'composition.json'),
      'composition.resolved references composition.json',
    ),
    req(
      'codex launch attach stream',
      Boolean(codexSession),
      'supported Codex runtime.session.started with durable transcript-backed artifact',
    ),
    req(
      'approval top lane',
      approvedRuntimeAction(events, runDir, shellMutationApproval, 'shell_mutation'),
      'approval.requested, approved approval.decided, and runtime.action.approved are ordered and bound to the same persisted approval artifact',
    ),
    req(
      'interrupt',
      Boolean(
        eventWith(
          ...
          (event) => event.payload.verb === 'interrupt' && event.payload.evidence_status === 'supported',
        ),
      ),
      'operation-specific interrupt proof must be supported',
    ),
    req(
      'resume',
      Boolean(
        eventWith(
          events,
          'runtime.lifecycle.supported',
          (event) => event.payload.verb === 'resume' && event.payload.evidence_status === 'supported',
        ),
      ),
      'operation-specific resume proof must be supported',
    ),
    req(
      'fork',
      Boolean(
        eventWith(
          events,
          'runtime.lifecycle.supported',
          (event) => event.payload.verb === 'fork' && event.payload.evidence_status === 'supported',
        ),
      ),
      'operation-specific fork proof must be supported',
    ),
    req(
      'parallel blackboard',
      memory.some(
        (fact) =>
          fact.layer === 'blackboard' &&
          fact.run_id === options.runId &&
          (fact.source_event_ids.length || fact.artifact_refs.length),
      ),
      'blackboard memory fact with provenance for this run',
    ),
    req(
      'sequential handoff',
      memory.some(
        (fact) =>
          fact.layer === 'sequential_handoff' &&
          fact.run_id === options.runId &&
          (fact.source_event_ids.length || fact.artifact_refs.length),
      ),
      'sequential handoff memory fact with provenance for this run',
    ),
    req(
      'hard gate',
      ledgerValid && hardGate.decision === 'PASS',
      'runtime hard gate passes for this run as a capability slice before final full-target claim',
    ),
    req(
      'learning memory',
      memory.some(
        (fact) =>
          fact.layer === 'module_learning' &&
          fact.run_id === options.runId &&
          (fact.source_event_ids.length || fact.artifact_refs.length),
      ),
      'module learning memory fact with outcome/provenance for this run',
    ),
    req(
      'review boundary',
      projected?.status === 'completed' && hasArtifact(runDir, 'review.md'),
      'run projection is completed and review.md exists',
    ),
    req(
      'commit approval boundary',
      Boolean(approvedBoundary(events, runDir, 'commit')),
      'commit approval request and approved decision are represented with persisted approval artifact',
    ),
    req(
      'push deploy approval boundary',
      Boolean(approvedBoundary(events, runDir, 'push_deploy')),
      'push/deploy approval request and approved decision are represented with persisted approval artifact',
    ),
    req(
      'ledger projection UI render agreement',
      ledgerValid && Boolean(projected) && hasArtifact(runDir, 'events.jsonl') && uiSmokePasses(runDir, options.runId),
      'ledger validates, projection contains this run, and server-rendered UI smoke confirms the same run/events are visible',
    ),
  ];
