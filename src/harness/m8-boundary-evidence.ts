...
export function appendM8BoundaryEvidence(options: {
  ...
  const runtimeApproval = recordApprovalRequest(
    options.root,
    options.agentDir,
    runDir,
    options.runId,
    'shell_mutation',
    'm8 runtime action approval',
  );
  ...
  const commitApproval = recordApprovalRequest(
    options.root,
    options.agentDir,
    runDir,
    options.runId,
    'commit',
    'm8 commit approval boundary',
  );
  append(runDir, {
    runId: options.runId,
    source: 'permission-broker',
  ...
  const pushApproval = recordApprovalRequest(
    options.root,
    options.agentDir,
    runDir,
    options.runId,
    'push_deploy',
    'm8 push/deploy approval boundary',
  );
  append(runDir, {
    runId: options.runId,
    source: 'permission-broker',
...
function recordApprovalRequest(
  ...
  writeFileSync(
    join(runDir, ref),
    JSON.stringify(
      { ...approval, source: 'createApproval', canonical_path: join(agentDir, 'approvals', `${approval.id}.json`) },
      null,
      2,
    ),
  );
...
function snapshotResolvedApproval(root: string, agentDir: string, runDir: string, approvalId: string): void {
  ...
  writeFileSync(
    join(runDir, 'approvals', `${approvalId}.json`),
    JSON.stringify(
      { ...approved, source: 'resolveApproval', canonical_path: join(agentDir, 'approvals', `${approvalId}.json`) },
      null,
      2,
    ),
  );
