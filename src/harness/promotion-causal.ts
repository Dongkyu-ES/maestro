...
export interface PromotionCausalReport {
  ...
  treatmentContextSha256: string;
  contextDeltaIsPromotionOnly: boolean;
  reason: string;
  runs: {
    baseline: HarnessRunReport;
    control: HarnessRunReport;
    treatment: HarnessRunReport;
  };
}
...
function promotionContext(promotion: { id: string; text: string }): string {
  return `Promotion ${promotion.id}:\n${promotion.text}`;
}

function runDir(root: string, report: HarnessRunReport): string {
...
function extractDecision(root: string, report: HarnessRunReport): string {
  ...
  if (jsonStart >= 0) {
    try {
      const parsed = JSON.parse(lastMessage.slice(jsonStart)) as { decision?: unknown };
      if (typeof parsed.decision === 'string' && parsed.decision.trim()) return parsed.decision.trim();
    } catch {
...
function contextBundle(root: string, report: HarnessRunReport): Record<string, unknown> | undefined {
...
function contextDeltaIsPromotionOnly(options: {
  ...
  return (
    options.baseline.contextSha256 === options.control.contextSha256 &&
...
function causalReason(options: {
  stableControl: boolean;
  changedTreatment: boolean;
  contextDeltaIsPromotionOnly: boolean;
}): string {
  if (!options.stableControl) return 'baseline and control decisions differ; run behavior is not stable';
  if (!options.contextDeltaIsPromotionOnly) return 'control and treatment context delta is not isolated to the promotion';
  if (!options.changedTreatment) return 'treatment decision did not differ from the stable baseline/control decision';
  return 'stable baseline/control plus promotion-only context delta changed the treatment decision';
}

export async function verifyPromotionCausal(options: PromotionCausalInput): Promise<PromotionCausalReport> {
  ...
  const report: PromotionCausalReport = {
    schema_version: 1,
