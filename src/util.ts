...
export function gitNoIndexPatch(workspace: string, file: string): string { try { return execFileSync('git', ['diff', '--binary', '--no-index', '--', '/dev/null', file], { cwd: workspace, encoding: 'utf8' }); } catch (err: any) { return String(err.stdout || ''); } }
export function normalizeNoIndexPatch(patch: string): string { return patch; }
export function patchTouchedFiles(patch: string): string[] { return patch.split('\n').filter((line) => line.startsWith('diff --git ')).map((line) => line.trim().split(/\s+/)[3] || '').filter(Boolean).map((file) => file.replace(/^b\//, '')); }
...
export interface EvidenceResult { ok: boolean; output: string; error?: string; }
export function gitEvidence(args: string[], cwd = process.cwd()): EvidenceResult { try { return { ok: true, output: execFileSync('git', args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }) }; } catch (err: any) { const error = String(err.stderr || err.message || err); return { ok: false, output: error, error }; } }
