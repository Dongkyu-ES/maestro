...
import { type CodexSessionRecord, sha256Text, upsertCodexSession } from './codex-session-registry.js';

export function discoverCodexTranscriptPath(
...
export function createCodexLaunchProof(options: {
  ...
  autoAttachCurrentSession?: boolean;
}): CodexLaunchProof {
  ...
  if (transcriptPath && existsSync(transcriptPath)) {
    ...
    status = 'supported';
    notes.push(
      'Existing Codex session transcript was attached as first-class launch/attach/stream evidence. Approval, interrupt, resume, and fork remain operation-specific unless separately proven.',
    );
