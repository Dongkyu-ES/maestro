...
export class ShellPrimitiveAdapter implements AgentRuntimeAdapter {
  ...
  approve(_sessionId: string, _approval: ApprovalDecision): Promise<RuntimeCommandResult> {
    return Promise.resolve(unsupportedResult('approve', 'shell'));
  }
  interrupt(_sessionId: string, _reason: string): Promise<RuntimeCommandResult> {
