...
export function writeProjectionSqlite(dbPath: string, projection: RuntimeProjection): void {
  ...
  const insertSession = db.prepare(
    'INSERT INTO runtime_sessions(run_id,session_id,adapter_kind,status,last_event_sequence,evidence_json) VALUES(?,?,?,?,?,?)',
  );
  const insertApproval = db.prepare(
    'INSERT INTO runtime_approvals(run_id,approval_id,status,sequence) VALUES(?,?,?,?)',
  );
...
export function readProjectionSqliteSummary(dbPath: string): {
  runs: number;
  sessions: number;
  approvals: number;
  labels: number;
