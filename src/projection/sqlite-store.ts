...
export function writeProjectionSqlite(dbPath: string, projection: RuntimeProjection): void {
  ...
  for (const run of projection.runs) {
    ...
    for (const session of run.sessions) insertSession.run(run.run_id, session.session_id, session.adapter_kind || null, session.status, session.last_event_sequence, JSON.stringify(session.evidence));
    for (const approval of run.approvals) insertApproval.run(run.run_id, approval.approval_id, approval.status, approval.sequence);
    for (const label of run.labels) insertLabel.run(run.run_id, label);
    for (const artifact of run.artifacts) insertArtifact.run(run.run_id, artifact);
  }
  db.close();
}

export function readProjectionSqliteSummary(dbPath: string): { runs: number; sessions: number; approvals: number; labels: number; artifacts: number } {
  const { DatabaseSync } = require('node:sqlite') as { DatabaseSync: any };
  const db = new DatabaseSync(dbPath, { readOnly: true });
  const count = (table: string) => Number(db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get().count || 0);
  const out = { runs: count('runtime_runs'), sessions: count('runtime_sessions'), approvals: count('runtime_approvals'), labels: count('runtime_labels'), artifacts: count('runtime_artifacts') };
  db.close();
  return out;
}
