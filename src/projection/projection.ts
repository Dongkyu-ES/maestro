...
export interface RuntimeRunProjection {
  ...
  approvals: { approval_id: string; status: string; sequence: number }[];
  artifacts: string[];
  labels: string[];
}
...
export function rebuildRuntimeProjection(events: RuntimeEventEnvelope[]): RuntimeProjection {
  ...
  for (const event of events) {
    ...
    const label =
      typeof event.payload.runtime_label === 'string'
        ? event.payload.runtime_label
        : typeof event.payload.adapter_kind === 'string'
          ? event.payload.adapter_kind
          : undefined;
    ...
    if (event.type === 'runtime.session.started') {
      ...
      session.adapter_kind = String(
        event.payload.adapter_kind || session.adapter_kind || event.source.replace('-adapter', ''),
      );
    ...
    if (event.type === 'approval.requested')
      run.approvals.push({
        approval_id: String(event.payload.approval_id || event.event_id),
        status: 'requested',
    ...
    if (event.type === 'approval.decided') {
      ...
      if (prior) {
        prior.status = String(event.payload.decision || 'decided');
