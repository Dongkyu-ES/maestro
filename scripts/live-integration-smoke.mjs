...
const child = spawn(process.execPath, [join(root, 'dist', 'cli.js'), 'web', '--host', '127.0.0.1', '--port', String(port)], { cwd: tmp, stdio: ['ignore', 'pipe', 'pipe'] });
try {
  await new Promise((resolve) => setTimeout(resolve, 800));
  const home = await fetch(`http://127.0.0.1:${port}/`);
  const html = await home.text();
  if (!html.includes('Your input / permissions') || !html.includes('Tool / permission boundary') || !html.includes('Agent / LLM work')) throw new Error('home UI lacks operator/agent/permission lanes');
  const csrf = html.match(/name="csrf" value="([^"]+)"/)?.[1];
  if (!csrf) throw new Error('missing csrf');
  const post = async (path, body) => fetch(`http://127.0.0.1:${port}${path}`, { method: 'POST', redirect: 'manual', headers: { 'content-type': 'application/x-www-form-urlencoded', origin: `http://127.0.0.1:${port}` }, body: new URLSearchParams({ csrf, ...body }) });
  let res = await post('/api/tasks', { title: 'live integration task' });
  if (res.status !== 303) throw new Error(`task create status ${res.status}`);
  const tasks = execFileSync(process.execPath, [join(root, 'dist', 'cli.js'), 'task', 'list'], { cwd: tmp, encoding: 'utf8' });
  ...
  const latest = execFileSync(process.execPath, [join(root, 'dist', 'cli.js'), 'run', 'latest'], { cwd: tmp, encoding: 'utf8' }).trim();
  ...
  if (!String(processJson.stdout || '').includes('Dominic Orchestration task adapter executed')) throw new Error('missing task adapter stdout');
  ...
  if (!runYaml.includes('status: "completed"') || !runYaml.includes('decision: "pass"')) throw new Error('run was not collected to pass');
  const detail = await (await fetch(`http://127.0.0.1:${port}/run/${latest}`)).text();
  if (!detail.includes('Run status summary') || !detail.includes('executor.process.json')) throw new Error('run detail lacks evidence summary');
  const artifact = { status: 'PASS', root: tmp, run_id: latest, exit_code: processJson.exit_code, decision: 'pass', natural_language_ignored: true, ui_permission_boundary: html.includes('Tool / permission boundary'), created_at: new Date().toISOString() };
