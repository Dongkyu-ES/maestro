...
- **Daemon** `src/harness/orchestrator-server.ts` + `warden orchestrate serve`: structured DAG over `POST /graph` (async 202 + jobId), thin deterministic router (declared kind → static purpose map → codex), `GET /jobs/:id` (with on-disk recovery after restart), SSE `GET /runs/:id/events`, loopback + auth. `warden orchestrate run --file` runs a DAG in-process.
