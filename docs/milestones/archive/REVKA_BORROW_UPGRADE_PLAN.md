...
- Each iteration's evidence is stored under an immutable per-iteration namespace (`runId/iterations/iter-k`) and is never overwritten (mirrors the immutability `selectExecuteCandidateByAcceptance` already relies on). **Correction (impl-verify panel):** the lifecycle ledger records ONE `phase.advanced` for the execute phase (per-iteration detail lives in `PhaseResult.iterations`, observability-only) — identical to fan-out, which also does not emit per-candidate ledger events. The earlier "distinct `phase.advanced` per iteration" wording is retracted; it is not what the code does and not required for the completion authority (recompute re-runs acceptance over the promoted canonical bytes).
...
1. **U1**: add `maxRefineIterations` to `OrchestratorSkillSpec` (+ JSON), generalize the execute phase into a bounded loop in `runOrchestratorSkill`; reuse `runAcceptanceCheck` as the gate and `selectExecuteCandidateByAcceptance`'s winner-by-acceptance rule across iterations; `recomputeCompletionFromLedger` unchanged (it already re-runs acceptance over the canonical execute refs). On exhaustion, promote the **last gradeable** attempt so recompute returns `failed`; only a run where NO iteration produced a gradeable artifact is `blocked` (recompute then `skipped`).
