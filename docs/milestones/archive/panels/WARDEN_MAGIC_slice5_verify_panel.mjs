#!/usr/bin/env node
// VERIFICATION panel on Warden Magic slice 5 (canary smokeProbe — consumption proof).
...
function seed() {
  const root = mkdtempSync(join(tmpdir(), 'magic-slice5-verify-'));
  execFileSync('git', ['init'], { cwd: root, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.email', 't@example.com'], { cwd: root });
  execFileSync('git', ['config', 'user.name', 'T'], { cwd: root });
  for (const f of ['smoke-probe.ts', 'smoke-probe.test.ts', 'inject.ts']) copyFileSync(`${REPO}/src/composition/${f}`, join(root, f));
  ...
  const diff = execFileSync('git', ['show', 'e3aa2a1'], { cwd: REPO, encoding: 'utf8', maxBuffer: 1024 * 1024 * 20 });
  writeFileSync(join(root, 'DIFF.patch'), diff);
  execFileSync('git', ['add', '.'], { cwd: root });
  execFileSync('git', ['commit', '-m', 'seed slice5 code'], { cwd: root, stdio: 'ignore' });
  return root;
}
...
async function main() {
  ...
  const report = await orch.runParallelWorkers({
    root, goal: 'verification panel on Warden Magic slice 5', concurrency: 3,
    workers: [
